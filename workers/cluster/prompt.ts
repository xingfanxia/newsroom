...
/**
 * Event-level commentary prompts.
 *
 * Stage D: generates editorial commentary at the event (cluster) level for
 * multi-source events (member_count >= 2). Per-item commentary in
 * workers/enrich/commentary.ts is skipped for items in multi-member clusters
 * — they defer here instead.
 *
 * The editorial voice and output schema (editorNote_{zh,en},
 * editorAnalysis_{zh,en}) match the per-item commentary produced by
 * workers/enrich/prompt.ts. The key difference: the input is "this EVENT
 * covered by N sources" rather than "this story from one source", and the
 * prompt includes the member list so the commentary can reference angles that
 * multiple sources cover — or angles that only one covers.
 */
