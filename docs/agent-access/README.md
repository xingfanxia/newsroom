...
- **Cookie/admin ok envelopes are centralized** — `session-auth`, `admin-auth`, and `admin-session-routes` reuse `lib/api/ok-response.ts`; protected admin leaf routes use `runAdminRoute` plus `adminRouteResult` for domain result branches and should not call `NextResponse.json({ ok: ... })` directly. Login/logout still own cookie setting through `freshAdminSessionCookie` / `expiredAdminSessionCookie`.
