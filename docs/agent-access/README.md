...
- **`lib/rss/render.ts` + `lib/rss/main-feed-meta.ts`** — shared RSS 2.0 XML/HTTP response envelope and main-locale feed metadata for `/api/feed/{locale}/rss.xml`, including XML escaping, safe `content:encoded` CDATA splitting, feed namespaces, extension elements, cache headers, content type, and lightweight markdown-ish HTML rendering. The main feed route, layout alternate links, home RSS button, and `/agents` cards reuse the same locale/path/title contract.
