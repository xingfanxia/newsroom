...
- **`lib/api/iteration-routes.ts`** — shared admin iteration-run lookup/apply/reject result helpers; the `[id]` routes keep only admin auth, route-id parsing, and response-envelope mapping.
...
- **`lib/api/saved-routes.ts`** — shared saved-item route payload helpers used by bearer-gated `/api/v1/saved` and MCP `ax_radar_save`; it owns saved lookup, agent saved-item serialization, save toggling, owner-aware collection assignment, assigned-collection payloads, and missing-item FK-to-`item_not_found` mapping while adapters keep their auth and response envelopes.
...
- `tests/api/feed-query-params.test.ts` — shared feed/search request helpers, parameter defaults, source filter validation, max-limit ceilings, tag parsing, and `FeedQuery` mapping
...
- `tests/api/saved-routes.test.ts` + `tests/api/saved-routes-source.test.ts` — saved list/write helper behavior and source wiring keep `/api/v1/saved` on shared list payload construction and keep `/api/v1/saved` plus MCP `ax_radar_save` on the same mutation path
...
- **Cookie/admin ok envelopes are centralized** — `session-auth`, `admin-auth`, and `admin-session-routes` reuse `lib/api/ok-response.ts`; they should not call `NextResponse.json({ ok: ... })` directly. Login/logout still own cookie setting through `freshAdminSessionCookie` / `expiredAdminSessionCookie`.
...
- **Feed/search query parsing is centralized** — `/api/public/feed`, `/api/public/search`, `/api/v1/feed`, and `/api/v1/search` call request helpers in `lib/api/feed-query-params.ts`; leaf routes do not construct `URL` objects or import query schemas. Only public/v1 max limits differ. Feed-facing tier values come from `VISIBLE_ITEM_TIERS`, highlight/deep-dive tier checks come from `HIGHLIGHT_ITEM_TIERS` / `isHighlightItemTier`, `today|archive` comes from `FEED_VIEWS`, `lexical|semantic` comes from `SEARCH_MODES`, and `source_group` / `source_kind` come from `SOURCE_GROUPS` / `SOURCE_KINDS`, all in `lib/types.ts`; MCP, `/skill.md`, and `/openapi.yaml` render or validate those same runtime tuples instead of repeating enum lists.
...
- **Bearer agent item serialization is shared across REST + MCP** — `/api/v1/feed`, `/api/v1/search`, MCP `ax_radar_feed`, and MCP `ax_radar_search` all use `toAgentApiItem`; `/api/v1/saved` gets its saved-item extension through `listSavedItemsRoutePayload` so the route does not import serializers directly.
- **Saved collection assignment is owner-aware** — `/api/v1/saved` writes and MCP `ax_radar_save` enter through `saveItemRoutePayload`, while browser move actions enter through `assignSavedItemCollection`; both paths reject another user's collection id before mutating assignment.
...
- **Event-member payload execution is shared across surfaces** — `/api/events/*`, `/api/public/events/*`, and `/api/v1/events/*` call `getEventMembersRequestPayload` for route-id parsing, locale query extraction/validation, DB lookup, and `{ cluster_id, members, total }` assembly. Shared helpers also own the UI/v1 `{ members, total }` envelope and public ETag signal parts. MCP `ax_radar_event_members` calls `getEventMembersPayload` directly after its Zod input validation. Positive route IDs still come from `lib/api/route-params.ts`; adapters keep only auth/rate-limit/cache/error-envelope choices.
