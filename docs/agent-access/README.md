...
- **`lib/api/v1-route.ts`** — shared bearer-gated route envelope for `/api/v1/*`. It bridges `requireApiToken(req)` into a handler callback and owns the plain JSON/error helpers (`v1Json`, `v1Error`, `v1InvalidQuery`, `v1InvalidQueryResult`, `v1ServerError`), leaving v1 route files responsible only for choosing request parsers and business helpers.
