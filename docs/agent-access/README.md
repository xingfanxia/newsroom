...
- `tests/api/mcp-contract-source.test.ts` — MCP feed/search stay wired to shared execution helpers, with feed using the shared v1 item serializer and search using the shared search payload serializer
...
- **Feed/search query parsing is centralized** — `/api/public/feed`, `/api/public/search`, `/api/v1/feed`, and `/api/v1/search` call request helpers in `lib/api/feed-query-params.ts`; leaf routes do not construct `URL` objects or import query schemas. Only public/v1 max limits differ. Feed-facing tier values come from `VISIBLE_ITEM_TIERS`, highlight/deep-dive tier checks come from `HIGHLIGHT_ITEM_TIERS` / `isHighlightItemTier`, `today|archive` comes from `FEED_VIEWS`, `lexical|semantic` comes from `SEARCH_MODES`, and `source_group` / `source_kind` come from `SOURCE_GROUPS` / `SOURCE_KINDS`, all in `lib/types.ts`; exact `source_id` filters win over source-group/source-kind buckets. MCP, `/skill.md`, and `/openapi.yaml` render or validate those same runtime tuples instead of repeating enum lists.
...
- **Bearer agent item serialization is shared across REST + MCP** — `/api/v1/feed` and MCP `ax_radar_feed` use `toAgentApiItem`; search rows reach the same item serializer through `toAgentSearchPayload`, and `/api/v1/saved` gets its saved-item extension through `listSavedItemsRoutePayload` so leaf routes do not import serializers directly.
...
- **Tweak validation is shared across browser + agent surfaces** — `lib/tweaks.ts` owns site-config option values and defaults, `lib/watchlist.ts` owns watchlist trim/lowercase/case-insensitive dedupe, and `/api/tweaks` plus `/api/v1/tweaks` both parse PATCH bodies and build DB patches through `lib/api/tweak-requests.ts`.
