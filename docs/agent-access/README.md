...
- **Event-member payload execution is shared across surfaces** — `/api/events/*`, `/api/public/events/*`, and `/api/v1/events/*` call `getEventMembersRequestPayload` for route-id parsing, locale query extraction/validation, DB lookup, and `{ cluster_id, members, total }` assembly. Shared helpers also own the UI/v1 `{ members, total }` envelope and public ETag signal parts. MCP `ax_radar_event_members` calls `getEventMembersPayload` directly after its Zod input validation. Positive route IDs still come from `lib/api/route-params.ts`; adapters keep only auth/rate-limit/cache/error-envelope choices, with the legacy UI route mapping domain failures through `plainRouteResult`.
