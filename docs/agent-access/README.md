...
- **`lib/types.ts`** — shared item tier, feed view, search mode, app/source locale, source group/kind, source-health status, and cadence runtime tuples used by REST/MCP schemas, public/agent item source fields, cluster lead-pick authority typing, `/skill.md`, `/openapi.yaml`, sitemap generation, DB enums, fetcher support checks, score parsing, and commentary workers.
