...
- **Tier-gated commentary (2026-05-08)**: Stage-4 commentary now branches by tier instead of running the full schema for every non-excluded item. `editor_note_*` (一句话点评) runs for every non-excluded item / event; `editor_analysis_*` (深度解读) **only** runs for tier `featured` / `p1`. Tier `all` items take the lighter `commentaryNoteSchema` LLM call (`COMMENTARY_NOTE_ONLY_SYSTEM`, ~85% smaller output). Cluster commentary path also extended to cover `event_tier='all'` — previously only featured/p1 events got commentary at all. Worker dispatch in `workers/enrich/commentary.ts` + `workers/cluster/commentary.ts`; backfill mirror in `scripts/ops/backfill-style.ts`. Policy spec: `modules/feed/runtime/policy/skills/editorial.skill.md` § "Tier gating".
