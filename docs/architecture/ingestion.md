...
- **Output**: `{ importance: int(0, 100), reasoning: string, tier: "featured" | "all" | "p1" | "excluded" }`.
  ...
  should import those tuples instead of rewriting tier arrays. The
  highlight/deep-dive subset (`featured` / `p1`) lives in
  `HIGHLIGHT_ITEM_TIERS` and `isHighlightItemTier`; feed serializers,
...
- **Embeddings (§2.4)**: `voyage-3 / text-embedding-3-large` — we picked **text-embedding-3-large native 3072 dims**, stored since the Turso migration as `F32_BLOB(3072)` (libSQL native vectors) plus a Matryoshka 256-dim `embedding_small` column carrying the DiskANN cosine index `items_embedding_small_idx` (3072-dim indexes are unbuildable on Turso; search re-ranks candidates with the full vector) — originally pgvector `halfvec(3072)` + HNSW.
...
- **AI HOT integration (2026-05-08, voice refreshed 2026-06-10, clustering corrected 2026-07-23)**: added pre-curated source `aihot-selected` (kind `aihot-api`) ingesting hourly from https://aihot.virxact.com; merge their structured `/api/public/daily` report into our daily column generator as a must-cover baseline (`newsletters.aihot_daily_payload` + `aihot_daily_date`). The hourly `mode=selected` endpoint yields individual article rows, so those rows participate in normal event clustering even though the collection is curated. Only rows that are themselves multi-topic digests (for example `ai-chatgroup-daily`) opt out. Voice prompts target a friend-sharing style: plain, useful, accurate, and low on AI/memo flavor. Historical design record: [`docs/aihot-integration/PLAN.md`](../aihot-integration/PLAN.md); this section is the current runtime summary. New env vars: `AIHOT_API_BASE_URL` + `AIHOT_API_USER_AGENT` (both with safe defaults). Operator scripts: `scripts/ops/backfill-style.ts` (cost-bounded re-enrich), `scripts/ops/import-aihot-daily-history.ts` (180-day daily history import), `scripts/ops/repair-aihot-daily-windows.ts` (dry-run-first cleanup for legacy placeholder rows whose period windows predate the shared 05:00Z helper), and `scripts/migrations/enable-aihot-event-clustering-20260723.ts` (dry-run-first correction of the production source flag).
- **AI HOT upstream outage signature (observed 2026-07-26 and 2026-08-01/02)**: AI HOT's daily report is generated once at ~00:00Z from their curated ("selected") pool and is only sometimes regenerated later (observed 12:22Z / 22:53Z reruns). When their curation layer stalls, `mode=selected` drops to a trickle (3 items on 8/1 vs ~15-20 healthy) and the daily report ships near-empty (2 sections / 2 items) — while `mode=all` (raw firehose) stays healthy (188 items on 8/1). Diagnosis rule: a thin daily ≠ site outage; compare `mode=all` counts for the same window before concluding anything. Our handling: `workers/newsletter/aihot-daily.ts` treats reports under `AIHOT_THIN_REPORT_ITEM_COUNT` (8) section+flash items as degraded — thin cached payloads stay refreshable (richer copy wins) and a `[aihot-daily]` degraded warn is logged; the daily column proceeds on our own pool regardless (baseline is advisory). The zylos `aihot-virxact-daily` Feishu digest (herdr) self-curates from `mode=all` when the report is thin.
...
- **Agent access — public anonymous mirror (2026-05-13, PR #36)**: opened a third integration track alongside bearer-gated `/api/v1/*` + `/api/mcp`. Public read-only endpoints under `/api/public/*` are anonymous, IP-rate-limited (`lib/rate-limit/public.ts`), weak ETag + If-None-Match → 304 (`lib/api/public-helpers.ts`), CORS open, and strip LLM-internal fields (raw reasoning, per-axis HKR `reasonsZh/En`). Discovery: hosted `/skill.md` (SKILL.md standard, installable into any SKILL-aware agent), `/openapi.yaml` (OpenAPI 3.1), `/robots.txt` (allow public surfaces, disallow admin/v1/cron), `/sitemap.xml` (Next.js metadata files `app/robots.ts` + `app/sitemap.ts`). New bilingual page `/{locale}/agents` with 3 tabs (Skill / RSS / REST API) modeled on AI HOT's `/agent-access`. Full endpoint inventory, rate limits, and contributor guide live in [`docs/agent-access/README.md`](../agent-access/README.md). No schema changes. No env-vars added.
