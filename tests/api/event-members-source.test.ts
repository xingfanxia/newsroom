...
describe("event member route source wiring", () => {
  test("all HTTP event-member routes share route param parsing", () => {
    for (const path of routePaths) {
      const source = read(path);

      expect(source).toContain("@/lib/api/event-members");
      expect(source).toContain("parseEventMemberRouteParams");
      expect(source).toContain("toEventMemberApiItems");
