...
| **LLM usage guardrails + observability** | Enrich claim/backoff fields prevent duplicate cron/backfill LLM spend; event-commentary cron only spends on active 24h events while historical sweeps stay operator-controlled; usage admin/API/MCP include all-time totals plus task/model breakdowns and recent-call model labels | ✅ shipped (2026-06-11; event-commentary cap 2026-06-12) |
...
| **Agent access — public mirror** | Anonymous `/api/public/*` read-only mirror with IP rate limit + weak ETag + CORS; full endpoint inventory and rate limits live in [`docs/agent-access/`](./docs/agent-access/). Discovery: hosted `/skill.md` (SKILL.md standard) + `/openapi.yaml` (OpenAPI 3.1) + `/robots.txt` + `/sitemap.xml`. Bilingual `/{locale}/agents` page with 3-tab UI (Skill / RSS / REST API). Bearer-gated `/api/v1/*` + `/api/mcp` retained for write actions + audit | ✅ shipped (2026-05-13, PR #36) |

Docs routing starts at [`docs/README.md`](./docs/README.md). Architecture map and ownership boundaries live in [`docs/architecture/overview.md`](./docs/architecture/overview.md). Current ingestion architecture, including AI HOT runtime behavior and blueprint deviations, lives in [`docs/architecture/ingestion.md`](./docs/architecture/ingestion.md). The shipped AI HOT design record is archived at [`docs/aihot-integration/PLAN.md`](./docs/aihot-integration/PLAN.md). Agent access surface is in [`docs/agent-access/`](./docs/agent-access/). Testing and local verification strategy lives in [`docs/testing/strategy.md`](./docs/testing/strategy.md). Current handoff notes are in [`docs/HANDOFF.md`](./docs/HANDOFF.md).
...
文档路由入口见 [`docs/README.md`](./docs/README.md)。架构总图和 ownership 边界见 [`docs/architecture/overview.md`](./docs/architecture/overview.md)。当前 ingestion 架构（含 AI HOT 运行时行为和蓝图偏差）见 [`docs/architecture/ingestion.md`](./docs/architecture/ingestion.md)。已上线的 AI HOT 设计记录归档在 [`docs/aihot-integration/PLAN.md`](./docs/aihot-integration/PLAN.md)。Agent 接入设计见 [`docs/agent-access/`](./docs/agent-access/)。当前会话交接记录见 [`docs/HANDOFF.md`](./docs/HANDOFF.md)。
